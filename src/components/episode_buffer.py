...
class ReplayBuffer(EpisodeBatch):
    def __init__(self, scheme, groups, buffer_size, max_seq_length, preprocess=None, device="cpu"):
        ...
        self.epoch_indices = []
        self.test_indices = []  # Will be set once and never changed
    ...
    #     """Shuffle indices to start a new epoch."""
    #     self.epoch_indices = np.random.permutation(self.episodes_in_buffer).tolist()

    def _prepare_epoch_indices(self, batch_size):
        """Shuffle indices to start a new epoch and reserve batch_size indices for testing."""

        # Keep the last `batch_size` elements for testing
        if not self.test_indices:  # Set test_indices only once
            shuffled_indices = np.random.permutation(self.episodes_in_buffer).tolist()
            # Keep the last `batch_size` elements for testing (test_indices won't change after this)
            self.test_indices = shuffled_indices[-batch_size:]
            # print("test indices ", self.test_indices)

        shuffled_indices = np.random.permutation(self.episodes_in_buffer).tolist()
        self.epoch_indices = [i for i in shuffled_indices if i not in self.test_indices]
        # print("epoch_indices ", len(self.epoch_indices))

    def sample_epoch(self, batch_size):
        """
        Samples a batch from the buffer, ensuring a full epoch is completed before restarting.
        """
        assert self.can_sample(batch_size)

        if not self.epoch_indices:  # Start a new epoch if all indices are used
            self._prepare_epoch_indices(batch_size)
            # print("we are building new epoch_indices ", len(self.epoch_indices))


        num_samples_to_return = min(batch_size, len(self.epoch_indices))
        # Get the required indices
        sampled_indices = [self.epoch_indices.pop(0) for _ in range(num_samples_to_return)]
        # print("sampled indices ", len(sampled_indices))
        # print("remaining indices ",len(self.epoch_indices))
        return self[sampled_indices]

    def sample_test_episode(self, batch_size, training=False):
        # if self.episodes_in_buffer> 2002 +128:
        if training:
            return self[self.test_indices]
        else:
            # print(len(self[-batch_size:]))
            # Print debug information
            # print("episode in buffer ", self.episodes_in_buffer)
            # print("batch size  ", batch_size)

            # Select the last `batch_size` episodes
            ep_ids = np.arange(self.episodes_in_buffer - batch_size, self.episodes_in_buffer)
            return self[ep_ids]
