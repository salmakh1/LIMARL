...
def my_main(_run, _config, _log):
    ...
    logger.info("################ The configuration is ##############")
    logger.info(config)
    print()
    logger.info("########################## config['seed']", config['seed'])
    np.random.seed(config["seed"])
    torch.manual_seed(config["seed"])
    config['env_args']['seed'] = config["seed"]
    exp_name = config["env"]
    run_name = config["env_args"]["map_name"]
    ...
    wandb.init(
        ...
        name=f"exp_name_{exp_name}_map_name_{run_name}_learner_{learner}_name_{name}_mixer_{mixer}_TS_{time_stamp}",
        config=config,
